...
def reduce_storeage(image):
    try:
        ...
        print(texts)
        cv2.imwrite("/tmp/unreduced.jpeg",image)
        while temp<100:
            image=cv2.imread("/tmp/unreduced.jpeg")
            cv2.imwrite("/tmp/reduced.jpeg",image,[int(cv2.IMWRITE_JPEG_QUALITY), temp])
            img=cv2.imread("/tmp/reduced.jpeg")
            os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = os.getenv('GOOGLE_APPLICATION_CREDENTIALS', 'credentials.json')
            client = vision.ImageAnnotatorClient()
